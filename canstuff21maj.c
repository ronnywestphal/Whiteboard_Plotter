...
#define CAN_DRAW_LINE       0x600
#define CAN_DRAW_CIRCLE     0x601
#define CAN_DRAW_SQUARE     0x602
#define CAN_DRAW_MARIO      0x603
#define CAN_TRAVEL          0X604
    ...
    switch(message.rx_sfid)
    {
        ...
        case CAN_TRAVEL:
            if(message.rx_ft == CAN_FT_DATA){
                ...
                travel(*pX,*pY,P_coordinates[0],P_coordinates[1]);
...
void pwm_up(void){
    ...
    timer_channel_output_pulse_value_config(TIMER4,TIMER_CH_1,(int)1001);
    delay_1ms(20);
}
void pwm_down(){
    int j=1001;
